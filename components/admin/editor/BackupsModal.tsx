...
import type { BackupItem, EventoConfig } from "@/lib/types";
...
export function BackupsModal({ open, onClose, eventoId }: Props) {
  ...
  const methods = useFormContext<EventoConfigInput>();
  ...
  async function handleRestore(key: string) {
    ...
    try {
      ...
      const configData: EventoConfig = await configRes.json();
      methods.reset(configData as EventoConfigInput);
