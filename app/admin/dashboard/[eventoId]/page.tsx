...
import { eventoConfigSchema, type EventoConfigInput } from "@/lib/schemas";
import { DEFAULT_EVENTO_CONFIG } from "@/lib/types";
...
import { InfoSection } from "@/components/admin/editor/sections/InfoSection";
import { FuncionesSection } from "@/components/admin/editor/sections/FuncionesSection";
import { NotasSection } from "@/components/admin/editor/sections/NotasSection";
import { GaleriaSection } from "@/components/admin/editor/sections/GaleriaSection";
import { SpotifySection } from "@/components/admin/editor/sections/SpotifySection";
import { ProductorasSection } from "@/components/admin/editor/sections/ProductorasSection";
import { SponsorsSection } from "@/components/admin/editor/sections/SponsorsSection";
import { MerchSection } from "@/components/admin/editor/sections/MerchSection";
...
export default function EventoEditorPage() {
  ...
  const methods = useForm<EventoConfigInput>({
    resolver: zodResolver(eventoConfigSchema),
    defaultValues: DEFAULT_EVENTO_CONFIG as EventoConfigInput,
    mode: "onBlur",
  });
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      reset(DEFAULT_EVENTO_CONFIG as EventoConfigInput);
    } finally {
  ...
  async function onSubmit(values: EventoConfigInput) {
      ...
      <form onSubmit={handleSubmit(onSubmit)} noValidate>
        ...
        <div className="sticky top-14 z-30 border-b border-line-light dark:border-line-dark bg-surface-light/95 dark:bg-surface-dark/95 backdrop-blur-sm">
          <div className="max-w-6xl mx-auto px-4 h-16 flex items-center gap-3">
            ...
            <span className="text-xs font-mono opacity-40 hidden md:block">
              {eventoId}
            </span>
        ...
        <div className="max-w-6xl mx-auto px-4 py-6">
          <div className="lg:grid lg:grid-cols-[200px_1fr] lg:gap-8">
            ...
            <div className="flex flex-col gap-10 min-w-0">
              <InfoSection eventoId={eventoId} />
              <FuncionesSection eventoId={eventoId} />
              <NotasSection />
              <GaleriaSection eventoId={eventoId} />
              <SpotifySection />
              <ProductorasSection eventoId={eventoId} />
              <SponsorsSection eventoId={eventoId} />
              <MerchSection eventoId={eventoId} />
            </div>
          </div>
        </div>
      </form>
